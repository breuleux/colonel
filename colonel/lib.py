...
class Sequence(CommonGateSpec):
    ...
    def __init__(self, n):
        ...
        super().__init__(
            ...
            deps_map = deps_map,
            triggers = [(i, {'out': REQ, i: AVAIL}, self.do)
                        for i in range(n)],
    ...
    def do(self, state, **args):
        ...
        return (((currentn + 1) % self.n, None),
                {'out': args[current] if currentn == self.n-1 else VOID},
                {current})
...
class Explode(CommonGateSpec):
    ...
    def __init__(self, n):
        ...
        super().__init__(
            ...
            starter = lambda: (0, None),
            deps_map = deps_map,
            triggers = [({name: REQ, 'input': AVAIL}, self.do)
    ...
    def do(self, state, input):
        ...
        return ((0, None),
                {name: input[i]
                 for i, name in enumerate(self.result_names)},
                {0})
...
class Join(FunctionGateSpec):

    def __init__(self, n):
        self.n = n
        super().__init__("Join{n}".format(n = n),
                         self.do,
